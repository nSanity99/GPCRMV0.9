...
if ($conn->connect_error) {
    ...
    header("Location: ../pages/gestioneutenze.php");
    exit;
}
...
elseif ($form_action === 'create_user_submit') {
    ...
    if (empty($username) || empty($password) || !in_array($ruolo, ['user', 'admin'])) {
        ...
        header("Location: ../pages/gestioneutenze.php?action=create_user");
        exit;
    }
    ...
    if ($stmt->execute()) {
        ...
        header("Location: ../pages/gestioneutenze.php?action=user_created");
    } else {
        $_SESSION['error_message_usermgmt'] = "Errore: l'username o l'email potrebbero già esistere.";
        header("Location: ../pages/gestioneutenze.php?action=create_user");
    }
