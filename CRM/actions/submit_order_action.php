...
if ($_SERVER["REQUEST_METHOD"] !== "POST") {
    ...
    header("Location: ../pages/form_page.php?status=order_error&message=" . urlencode("Metodo di richiesta non valido."));
    exit;
}
...
if (empty($id_utente_richiedente) || empty($nome_richiedente) || empty($centro_costo)) {
    ...
    header("Location: ../pages/form_page.php?status=order_error&message=" . urlencode("Dati principali mancanti."));
    exit;
}
...
if (empty($prodotti) || !is_array($prodotti)) {
    ...
    header("Location: ../pages/form_page.php?status=order_error&message=" . urlencode("Nessun prodotto specificato nella richiesta."));
    exit;
}
...
if ($conn->connect_error) {
    ...
    header("Location: ../pages/form_page.php?status=order_error&message=" . urlencode("Errore di connessione al database."));
    exit;
}
...
try {
    ...
    header("Location: ../pages/form_page.php?status=order_success");
    exit;
    ...
    header("Location: ../pages/form_page.php?status=order_error&message=" . $error_message_url);
    exit;
} finally {
